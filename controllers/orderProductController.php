<?php
	...
	if (!isset($_POST['action'])) {
		...
		if ($_POST['action']=='view') {
			...
			$totalFiltered = $totalData;  // wheeeeeen there is no search parameter then total number rows = total number filtered rows.
			// pr($totalFiltered);
			...
			while( $row=mysqli_fetch_assoc($query) ) {  // preparing an array
				...
				$users = getDataByParam('users','user_id',$row['cashier_id']);

				$nestedData[] = $users['data']['name'];
				$nestedData[] = $row["code"];
				$nestedData[] = $row["date"];
				$nestedData[] = $status;
				$nestedData[] = '
			...
			$data = getDataByParam('product_wardrobes', 'product_id','');
			// pr($data);
			$outArr=[
				'status'=>$data['status'],
				'data'	=>$data['data']
